<template>
  <div class="container mx-auto p-4">
    <h1 class="text-4xl font-bold text-center mb-8">Todas as Categorias</h1>
    ...
    <div v-else-if="categories.length > 0" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      <div v-for="category in categories" :key="category.slug"
           @click="navigateToCategory(category.slug)"
           class="bg-orange-100 text-black-800 p-6 rounded-lg shadow-md cursor-pointer
                  hover:bg-orange-200 hover:text-black-900 transition-colors duration-200
